...
{
  scream_require_msg (grid!=nullptr, "Error! Invalid grid pointer.\n");
  m_num_cols  = grid->get_num_local_dofs();
  m_grid_name = grid->name();

  scream_require_msg(grid->type()==GridType::SE_NodeBased,
                     "Error! Surface coupling only implemented for SE_NodeBased grids.\n"
                ...
                const int vecComp)
{
  ...
  scream_require_msg (m_state!=RepoState::Closed, "Error! Registration phase has already ended.\n");

  // Check that we still have room
  scream_require_msg(m_num_imports<m_scream_imports_host.extent_int(0),
                     "Error! Imports view is already full. Did you call 'set_num_fields' with the wrong arguments?\n");

  // Check that input field is valid
  scream_require_msg (field.is_allocated(), "Error! Import field view has not been allocated yet.\n");
  
  const std::string& fgn = field.get_header().get_identifier().get_grid_name();
  scream_require_msg (fgn==m_grid_name,
                      "Error! Input field is defined on the wrong grid:\n"
                      "         expected grid name: " + m_grid_name + "\n"
                      "         field grid name: " + fgn + "\n");

  scream_require_msg(cpl_idx>=0, "Error! Input cpl_idx is negative.\n");

  // Check that this cpl_idx wasn't already registered
  for (int i=0; i<m_num_imports; ++i) {
    scream_require_msg(cpl_idx!=m_scream_imports_host(i).cpl_idx,
  ...
  switch (layout.rank()) {
    case 1:
      scream_require_msg(vecComp==-1, "Error! Vector component specified, but field '" + fname + "' is a 2d scalar.\n");
      info.col_size = 1;
      break;
    case 2:
      scream_require_msg(tags[1]==VL || tags[1]==VAR || tags[1]==CMP,
                         "Error! Unexpected tag '" + e2str(tags[1]) + "' for second dimension of export field '" + fname + "'.\n");
      scream_require_msg(tags.back()!=VL || vecComp==-1, "Error! Vector component specified, but field '" + fname + "' is a 3d scalar.\n");

      info.col_size = alloc_prop.get_last_dim_extent<Real>();
      info.col_offset = vecComp;
      break;
    case 3:
      scream_require_msg(tags[1]==VAR || tags[1]==CMP,
                         "Error! Unexpected tag '" + e2str(tags[1]) + "' for second dimension of export field '" + fname + "'.\n");
      scream_require_msg(tags[2]==VL,
                         "Error! Unexpected tag '" + e2str(tags[2]) + "' for third dimension of export field '" + fname + "'.\n");
      scream_require_msg(vecComp>=0, "Error! Vector component not specified for 3d vector field '" + fname + "/.\n");

      info.col_size = layout.dim(1)*alloc_prop.get_last_dim_extent<Real>();
      info.col_offset = vecComp;
      break;
    default:
      scream_error_msg("Error! Unsupported field rank for import field '" + fname + "'.\n");
  }
                 ...
                 const int vecComp)
{
  ...
  // Check that we still have room
  scream_require_msg(m_num_exports<m_scream_exports_host.extent_int(0),
                     "Error! Exports view is already full. Did you call 'set_num_fields' with the wrong arguments?\n");

  // Check that input field is valid
  scream_require_msg (field.is_allocated(), "Error! Export field view has not been allocated yet.\n");
  ...
  for (int i=0; i<m_num_exports; ++i) {
    scream_require_msg(cpl_idx!=m_scream_exports_host(i).cpl_idx,
...
{
  // Two separate checks rather than state==Open, so we can print more specific error messages
  scream_require_msg (m_state!=RepoState::Clean,  "Error! Registration phase hasn't started yet.\n");
  scream_require_msg (m_state!=RepoState::Closed, "Error! Registration phase has already ended.\n");
  ...
  for (int i=0; i<m_num_imports; ++i) {
    scream_require_msg (m_scream_imports_host(i).data!=nullptr,
                        "Error! No field set for import index " + std::to_string(i) + ".\n");
  }
  for (int i=0; i<m_num_exports; ++i) {
    scream_require_msg (m_scream_exports_host(i).data!=nullptr,
                        "Error! No field set for import index " + std::to_string(i) + ".\n");
  }
  ...
  scream_require_msg(cpl_exports_ptr!=nullptr, "Error! Data pointer for exports is null.\n");
  scream_require_msg(cpl_imports_ptr!=nullptr, "Error! Data pointer for imports is null.\n");
