...
void SurfaceCoupling::register_import_field (const import_field_type& field) {
  scream_require_msg (m_state==RepoState::Open, "Error! Registration phase has already ended.\n");
...
void SurfaceCoupling::registration_ends () {
  ...
  for (const auto& it : m_imports) {
    scream_require_msg (it.second.data!=nullptr,
                        "Error! No data pointer set for '" + it.first + "'.\n");
    scream_require_msg (it.second.field.get_view().data()!=nullptr,
