...
public:
  ...
  bool has_tag (const FieldTag t) const { return util::contains(m_tags,t); }
...
inline int FieldLayout::dim (const FieldTag t) const {
  auto it = util::find(m_tags,t);

  // Check if found
  scream_require_msg(it!=m_tags.end(), "Error! Tag '" + e2str(t) + "' not found.\n");

  // Check only one tag (no ambiguity)
  scream_require_msg(util::count(m_tags,t)==1,
